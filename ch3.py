...
# reversed是一个生成器，如果没有示例化用list函数的时候，
# 并不会产生一个倒序的列表

# 3.1.4 字典
# dict字典可能是python内建数据结构中最重要的。更为常见
# 的名字是哈希表或者是关联数组。字典是拥有灵活尺寸的键值
# 对集合，其中键和值都是Python对象。用{}是创建字典的一种
# 方式，在字典中用逗号将键值对分隔
empty_dict={}
d1={'a':'some value','b':[1,2,3,4]}
d1
# 可以访问、插入或设置字典中的元素，就像访问列表和元组中
# 的元素一样
d1[7]= 'an integer'
d1
d1['b']
# 可以用检查列表或元组中是否含有一个元素的相同语法来检查
# 字典是否含有一个键：
'b' in d1
# 可以使用del关键字或者pop方法删除值，pop方法会在删除的同时
# 返回被删的值，并删除键
d1[5]='some value'
d1
...
ret
d1
# keys方法和values方法会分别提供字典键、值的迭代器。然而键值
# 对并没有特定的顺序，这些函数输出的键、值都是按照相同的顺序
list(d1.keys())
list(d1.values())
# 可以使用update方法将两个字典合并
d1.update({'b':'foo','c':12})
d1
# update方法改变了字典中元素位置，因此对于任何原字典中已经存在的
# 键，如果传给update方法的数据也含有相同的键，则它的值会被覆盖

# 3.1.4.1 从序列生成字典
# 通常情况下，你会有两个序列想要在字典中按元素配对。起初，你可能
# 会写出这样的代码